...
```{r}
YLD_NT_Chorio1<-burden(NT_Chorio1, DW_chorio1, A=0, L = Duration_LE_Bi, K=K, r=r, a=0)
```
...
```{r}
YLD_NT_Chorio2<-burden(NT_Chorio2, DW_chorio2, A=10, L = Duration_later_Bi, K=K, r=r, a=10)
```
...
```{r}
YLD_NT_hydrocephalus<-burden(NT_hydrocephalus, DW_Hydro, A=0, L = Duration_LE_Bi, K=K, r=r, a=0)
```
...
```{r}
YLD_NT_ICC<-burden(NT_IC_calcification, DW_ICC, A=0, L = Duration_LE_Bi, K=K, r=r, a=0)
```
...
```{r}
YLD_NT_CNS<-burden(NT_CNS_abnorm, DW_CNSab, A=0, L = Duration_LE_Bi, K=K, r=r, a=0)
```
...
```{r}
YLL_NT_neo_death<-burden(NT_Neon_deaths, DW_Neon, A=0, L = Duration_GBD, K=K, r=r, a=0)
```
### YLL for foetal loss during pregnancy
```{r}
YLL_floss<-burden(Nfetalloss, DW_Floss, A=0, L = Duration_GBD, K=K, r=r, a=0)
```
...
```{r}
YLD_Treat_chorio1<-burden(NTreat_Chorio1, DW_chorio1, A=0, L = Duration_LE_Bi, K=K, r=r, a=0)
```
...
```{r}
YLD_Treat_chorio2 <-burden(NTreat_Chorio2, DW_chorio2, A=10, L = Duration_later_Bi, K=K, r=r, a=10)
```
...
```{r}
YLD_Treat_hydroc <-burden(NTreat_hydroc, DW_Hydro, A=0, L = Duration_LE_Bi, K=K, r=r, a=0)
```
...
```{r}
YLD_Treat_ICC <-burden(NTreat_IC_calc, DW_ICC, A=0, L = Duration_LE_Bi, K=K, r=r, a=0)
```
...
```{r}
YLD_Treat_CNS <-burden(NTreat_CNS_ab, DW_CNSab, A=0, L = Duration_LE_Bi, K=K, r=r, a=0)
```
...
```{r}
YLL_Treat_neo_death <-burden(NTreat_Neon_deaths, DW_Neon, A=0, L = Duration_GBD, K=K, r=r, a=0)
```

### DISABILITY ADJUSTED LIFE YEARS_Total
...
#### Sensitivity analysis for TOTAL DALYs including foetal loss

```{r}
sa_src(DALY1,
       cbind(Pseroneg, Pserop, PTTMoth_fet, P_fetalloss, Chorio1, 
             Chorio2,Hydrocephalus,IC_Calcification, CNS_ab, Neon_deaths,PNtreat_toxo))

sa_daly1_pcc <-
  sa_pcc(DALY1,
         cbind(Pseroneg, Pserop, PTTMoth_fet, P_fetalloss, Chorio1, 
               Chorio2,Hydrocephalus,IC_Calcification, CNS_ab, Neon_deaths, PNtreat_toxo, PTreat_toxo))
sa_daly1_pcc
## tornado graph
daly1_items <-
  c("Proportion of seropositive","Proportion of foetal loss", "Proportion of neonatal deaths", "Proportion of CNS abnormalities", 
    "Proportion of hydrocephalus", "Proportion of chorio 1",
    "Proportion of seronegative", "Proportion of ICC", "Proportion of women treated","Seroconversion rate", "Proportion of chorio 2", "Proportion of women untreated")
...
### Sensitivity for DALYS without foetal loss
```{r}
sa_src(DALY2,
...
## tornado graph
daly2_items <-
  c("Proportion of seropositive", "Proportion of neonatal deaths", "Proportion of CNS abnormalities",  "Proportion of hydrocephalus", "Proportion of chorio 1", "Proportion of ICC", "Proportion of women treated", "Proportion of women untreated", "Seroconversion rate", "Proportion of chorio 2", "Proportion of seronegative")
...
sa_src(usd_cost_total,
       cbind(Pserop, Pseroneg, n_visit_med, 
             Cconsultation, Cost_diagnostic, Cost_spiramycin, Duration_treat, HC_Women_treat))
sa_cost_pcc <-
...
## tornado graph
cost_items <-
c("Cost diagnostic", "Cost consultation", "Duration of treatment", "Cost medicine", "Number of hospital visits", "Proportion of women treated", "Seroprevalence negative","Proportion of seropositive")
